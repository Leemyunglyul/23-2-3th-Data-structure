...
void task_2(ofstream &fout, const char *tree_string) {
    ...
    try {
        ...
        tree.general_tree(output, temp, 0);
        fout << output << endl;
    } catch (const char *e) {
        cerr << e << endl;
    }
...
void task_3(ofstream &fout, const char *tree_string,
    ...
    try {
        ...
        if (!strcmp(traverse_mode, "preorder")) {
            fout << tree.preOrder(temp, 0) << endl;
        } else if (!strcmp(traverse_mode, "postorder")) {
            fout << tree.postOrder(temp, 0) << endl;
        } else if (!strcmp(traverse_mode, "inorder")) {
            fout << tree.inOrder(temp, 0) << endl;
        } else if (!strcmp(traverse_mode, "levelorder")) {
            fout << tree.levelOrder(temp,0) << endl;
        } else {
...
string buildFromPreorder(int inLeft, int inRight) {
    ...
    while (inorder[n])
        n++;
    for(i=0;i<n;i++)
      if(order[inLeft]==inorder[i])
        break;
    if(i>=n)
      return "error";
    for(j=0;j<n;j++)
      if(order[j]==inorder[i-1])
        break;
    if(j>=n)
      return "error";
    
    list+='(';
    temp=buildFromPreorder(inLeft+1, j);
    if(temp=="error")
      return "error";
    list+=temp;
    list+=')';
    list+='(';
    temp=buildFromPreorder(j+1, inRight);
    if(temp=="error")
      return "error";
    list+=temp;
    list+=')';
...
string buildFromPostorder(int inLeft, int inRight) {
    ...
    list+='(';
    temp=buildFromPostorder(inLeft, j-1);
    if(temp=="error")
      return "error";
    list+=temp;
    list+=')';
    list+='(';
    temp=buildFromPostorder(j, inRight-1);
    if(temp=="error")
      return "error";
    list+=temp;
    list+=')';
...
void task_4(ofstream &fout, const char* argv[]) {
    ...
    while (order[m])
        m++;
    for (int i = 0; i < n; i++) {
        if(inorderIdx[order[i]] ==-1 || idx[order[i]]!=0){
          fout << "error" << endl;
          return;
        }
        idx[order[i]]=1;
